...
const CreditCard = ({ debitcard, creditCard, creditcard } : any) => {
    ...
    return (
        <LinearGradient colors={debitcard ? ['#50B0BF', '#50B0BF'] : ['#FFA048', '#FFA048']}
            style={{ height: creditCard ? 180 : 150, width: creditCard ? '100%' : 277, padding: 15,borderRadius:15 }}
        >
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                    {creditCard ?
                        null
                        :
                        <View style={{ height: 20, width: 20, borderRadius: 50, borderWidth: 1, borderColor: COLORS.white, alignItems: 'center', flexDirection: 'row', justifyContent: 'center',backgroundColor:creditcard ? COLORS.card : undefined }}>
                            <View style={{ height: 12, width: 12, backgroundColor:creditcard ? COLORS.primary : '#fff', borderRadius: 50 }}></View>
                        </View>
                    }
                    {debitcard ?
